...
public class Jidouhannbaiki {
	...
	private static void showEndMessage(int total, Item[] a) {
		//売上表示
		System.out.println("売上:" + total + "円");
	}

	private static int sell(Item[] items, int select) {
		//商品が売れる、売り上げが増える
		System.out.println();
		//売り上げの計算totalに計上
	...
	//売れる場合にはCanSellにて計上していく
	private static boolean canSell(Item[] items, int select) {
		//在庫がある状況
	...
	private static void showMenu(Item[] a) {
		//商品一覧を表示
	...
	//在庫一覧を表示
	 public static void setShowZaiko(Item[] a) {
		for (int i = 0; i < a.length; i++) {
			System.out.println(a[i].name + a[i].stock + "本");
		}
	}
